...
class Productscontroller{
	 ...
	 */
	public function __construct( PHPAppRegistry $registry, $directCall )
	{
		...
		{	
			...
			////print_r('YOU ARE HERE...'.$urlBits[1]);
	...
	/*	 	echo '<pre>If is valid...';
			//print_r($productData);
			echo 'end of productData.</pre>';*/ 
			...
			{
				//print_r('<br/>In stock variable gretter than zero...');
				$this->registry->getObject('template')->addTemplateBit( 'stock', 'instock.tpl.php' );
			}
			else
			{
				//print_r('<br/>Add template stock empty...');
			...
			if( $this->model->hasAttributes() )
			{
				...
				if( $this->model->hasCustomTextInputs() )
				{
					if( $this->model->allowUploads() )
					{
						...
						//print_r('<pre>FIELD DATA: '.$fieldsdata);print_r('</pre');
					...
					{
						$fieldsdata = $this->model->getCustomTextInputs();
						//print_r('<br/>FIELD DATA: '.$fieldsdata);
						$tags = array();
						...
						$cache = $this->registry->getObject('db')->cacheData( $tags );
						$this->registry->getObject('template')->getPage()->addTag('fields', array( 'DATA', $cache ) );
						//print_r('<br/>Has attributes-custom option');
			...
			{
				//print_r('<br/>Basic product template');
	...
	private function listProducts()
	{
		if( $this->filterSQL == '' )
		{
			$sql = "SELECT p.price as product_price, v.name as product_name, c.path as product_path, p.weight as product_weight, p.stock as product_stock FROM content c, content_versions v, content_types_products p WHERE  p.content_version=v.ID AND v.ID=c.current_revision AND c.active=1 ";
		}
		else
		{
			$sql = $filterSQL;
	...
	private function searchProducts()
	{
		...
		if( isset( $_POST['product_search'] ) && $_POST['product_search'] != '' )
		{
			...
			//print_r('SEARCHING PRODUCT FOR... '.$_POST['product_search']);
			...
			else
			{
				...
				var_dump('Returned something(s)...'.$cache);

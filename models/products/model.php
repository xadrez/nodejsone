...
class Product{
	...
	public function __construct( PHPAppRegistry $registry, $productPath )
	{
		$this->registry = $registry;
		if( $productPath != '' )
		{
			$productPath = $this->registry->getObject('db')->sanitizeData( $productPath );
			$productQuery = "SELECT v.name as product_name, c.ID as product_id, p.allow_upload as allow_upload, p.custom_text_inputs as custom_text_inputs, 
			( SELECT sum(rating)/count(*) 
			FROM content_ratings 
			WHERE contentID=c.ID ) as rating, (SELECT GROUP_CONCAT( a.name,'--AV--', av.ID, '--AV--', av.name SEPARATOR '---ATTR---' ) 
			FROM product_attribute_values av, product_attribute_value_association ava, product_attributes a 
			WHERE a.ID = av.attribute_id AND av.ID=ava.attribute_id AND ava.product_id=c.ID ORDER BY ava.order ) AS attributes, p.image as product_image, p.stock as product_stock, p.weight as product_weight, p.price as product_price, p.SKU as product_sku, p.featured as product_featured, v.heading as product_heading, v.content as product_description, v.metakeywords as metakeywords, v.metarobots as metarobots, v.metadescription as metadescription 
			FROM content_versions v, content c, content_types t, content_types_products p 
			WHERE c.active=1 AND c.secure=0 AND c.type=t.ID AND t.reference='product' AND p.content_version=v.ID AND v.ID=c.current_revision AND c.path='{$productPath}'";
			...
			if( $this->registry->getObject('db')->numRows() == 1 )
			{
				...
				//print_r('<br/>This product is active');
				// grab the product data, and associate it with the relevant fields for this object
				$data = $this->registry->getObject('db')->getRows();
				//print_r('Here is data attributes :'.$data['attributes'].' END of data');
				if( $data['attributes'] != '' )
				{
					$this->hasAttributes = true;
					/*print_r('<br/>This product has attributes<pre>');
					var_dump($data);
					print_r('</pre>');*/
					...
					//print_r('<pre>' . print_r( $this->attributes, true ) . '</pre>');
